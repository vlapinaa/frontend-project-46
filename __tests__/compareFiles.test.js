...
import {
  resultStylish,
  resultPlain,
} from "../__fixtures__/compareFiles.expect.js";
...
test("compare files JSON", () => {
  expect(stylish(compareJSON)).toBe(resultStylish);
  expect(plain(compareJSON)).toBe(resultPlain);
});
...
test("compare files YAML", () => {
  expect(stylish(compareYML)).toBe(resultStylish);
  expect(plain(compareYML)).toBe(resultPlain);
});
